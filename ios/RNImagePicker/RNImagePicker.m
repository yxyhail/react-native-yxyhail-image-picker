...
RCT_REMAP_METHOD(uploadFiles,
  ...
  self.resolveBlock = resolver;
  self.rejectBlock = rejecter;
  [self uploadFiles:url files:files params:params headers:headers];
}
...
/*
 上传文件
 */
-(void) uploadFiles: (NSString *) url files:(NSArray *) files params:(NSDictionary *) params headers:(NSDictionary *) headers{
  ...
  [files enumerateObjectsUsingBlock:^(id _Nonnull file,NSUInteger index, BOOL * _Nonnull stop) {
    NSMutableString *fileDataStr = [NSMutableString string];
    if(index !=0){
      [fileDataStr appendString:@"\r\n"];
    }
    [fileDataStr appendFormat:@"--%@", Kboundary];
    [fileDataStr appendString:@"\r\n"];
    [fileDataStr appendFormat:@"Content-Disposition: form-data; name=%@; filename=%@\r\n", @"file", file[@"name"]];
    [fileDataStr appendString:@"Content-Type: application/octet-stream\r\n"];
    [fileDataStr appendString:@"\r\n"];
    [requestData appendData:[fileDataStr dataUsingEncoding:NSUTF8StringEncoding]];
    // 拼接文件的二进制数据
  ...
  [params enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, id  _Nonnull value, BOOL * _Nonnull stop) {
    ...
    [requestData appendData:[mString dataUsingEncoding:NSUTF8StringEncoding]];
  }];
